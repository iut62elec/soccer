...
export default function SFworkflowUpdateForm(props) {
  const {
    id: idProp,
    sFworkflow: sFworkflowModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = sFworkflowRecord
      ? { ...initialValues, ...sFworkflowRecord }
      : initialValues;
    setSFARN(cleanValues.SFARN);
    setStatus(cleanValues.status);
    setFinal_highlight(cleanValues.final_highlight);
    setFinal_original(cleanValues.final_original);
    setErrors({});
  };
  const [sFworkflowRecord, setSFworkflowRecord] =
    React.useState(sFworkflowModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(SFworkflow, idProp)
        : sFworkflowModelProp;
      setSFworkflowRecord(record);
    };
    queryData();
  }, [idProp, sFworkflowModelProp]);
  React.useEffect(resetStateValues, [sFworkflowRecord]);
  const validations = {
    SFARN: [{ type: "Required" }],
    status: [],
    final_highlight: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            SFworkflow.copyOf(sFworkflowRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "SFworkflowUpdateForm")}
      {...rest}
    >
      <TextField
        label="Sfarn"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || sFworkflowModelProp)}
